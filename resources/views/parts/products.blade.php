      ...
      <div class="row">
		...
		@foreach(App\Category::sale() as $product)
          <div class="product-preview category3">
            <div class="preview animate scale"> <a href="{{ URL::to('product', $product->id) }}" class="preview-image"><img class="img-responsive animate scale cartUrl {{$product->id}}" src="{{ URL::asset('/') }}{{$product->url}}" width="270" height="328" alt=""></a>
              <ul class="product-controls-list right">
                <li><span class="label label-sale">SALE</span></li>
                <li><span class="label">-10%</span></li>
				<li><span class="label CartAlert {{$product->id}}">Đã Thêm</span></li>
              </ul>
              <ul class="product-controls-list right hide-right">
                <li class="top-out"></li>
                <li><a href="{{$product->id}}" class="cart" title="Thêm vào giỏ"><span class="icon-basket"></span></a></li>
              </ul>
            ...
            <h3 class="title cartName {{$product->id}}">{{$product->name}}</h3>
            <span class="price old">{{$product->price}}</span> <span class="price new lastprice {{$product->id}}"><?php echo 0.9*$product->price ;?></span> </div>
        @endforeach  
		@foreach(App\Category::bestSeller() as $product)
          <div class="product-preview category1">
            ...
            <h3 class="title cartName {{$product->id}}">{{$product->name}}</h3>
           <span class="price lastprice {{$product->id}}">{{$product->price}}</span> </div>
        @endforeach 
